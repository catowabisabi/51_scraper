...
class AutoScraper(BaseScraper):
    ...
    def _extract_price(self, soup: BeautifulSoup) -> float:
        """提取價格"""
        price_elem = soup.find(class_=re.compile(r'price|amount|cost'))
        if price_elem:
    ...
    def _extract_vin(self, soup: BeautifulSoup) -> str:
        """提取VIN碼"""
        text = soup.get_text()
        vin_match = re.search(r'VIN[：:\s]*([A-HJ-NPR-Z0-9]{17})', text, re.I)
        if vin_match:
    ...
    def _extract_seller_name(self, soup: BeautifulSoup) -> str:
        """提取賣家名稱"""
        seller_elem = soup.find(class_=re.compile(r'seller|dealer|owner'))
        if seller_elem:
            return self.clean_text(self.extract_text(seller_elem))[:100]
        return None
    ...
    def _extract_location(self, soup: BeautifulSoup) -> str:
        """提取位置"""
        loc_elem = soup.find(class_=re.compile(r'location|address|area'))
        if loc_elem:
    ...
    def save_item(self, data: Dict) -> bool:
        ...
        try:
            cursor.execute("""
                ...
                (listing_id, url, title, make, model, year, price, mileage,
