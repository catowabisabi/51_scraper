...
class HouseScraper(BaseScraper):
    ...
    def _parse_api_property(self, prop: Dict, transaction_type: int) -> Optional[Dict]:
        ...
        # 解析交易類型 - 使用 API 返回的值
        actual_transaction_type = prop.get('transactionType', transaction_type)
        listing_type = self.TRANSACTION_TYPES.get(actual_transaction_type, '買賣')
        
        # 價格單位
        price = prop.get('listingPrice', 0)
        price_unit = '月' if actual_transaction_type == 2 else '總價'
