...
class EventScraper(BaseScraper):
    ...
    SCRAPER_NAME = "event"
    BASE_URL = "https://info.51.ca"  # 活動在 info.51.ca 子域名
    URL_TYPE = "event"
    ...
    def get_start_urls(self) -> List[str]:
        ...
        return [
            ...
            f"{self.BASE_URL}/events?page=3",
        ]
    
    def is_list_page(self, url: str) -> bool:
        """判斷是否為列表頁面"""
        # 詳情頁: /events/posts/數字
        if re.search(r'/events/posts/\d+', url):
            return False
        return True
    ...
    def parse_list_page(self, html: str, url: str) -> List[Dict]:
        ...
        # 方法3: 通用連結搜索 - 匹配 /events/posts/數字
        event_links = soup.find_all('a', href=re.compile(r'/events/posts/\d+'))
    ...
    def parse_detail_page(self, html: str, url: str) -> Optional[Dict]:
        ...
        if card:
            ...
            for dt in dts:
                ...
                if dd:
                    ...
                    if '活动时间' in dt_text or '活動時間' in dt_text:
                        event_time_display = dd_text
                    elif '开始时间' in dt_text or '開始時間' in dt_text:
                        start_time = dd_text
                    elif '结束时间' in dt_text or '結束時間' in dt_text:
                        end_time = dd_text
        
        # 如果沒有開始時間但有活動時間顯示，用它作為開始時間
        if not start_time and event_time_display:
            start_time = event_time_display
        
        # 地區: dt:contains("所在地区") + dd
        region = self._extract_dd_value(card, ['所在地区', '所在地區'])
        
        # 聯絡人: dt:contains("联系人") + dd
        contact_person = self._extract_dd_value(card, ['联系人', '聯繫人', '聯絡人'])
        
        # 電話: dt:contains("联系电话") + dd 或 .events-card a.phone
        phone = self._extract_dd_value(card, ['联系电话', '聯繫電話', '電話'])
        if not phone and card:
            phone_elem = card.select_one('a.phone')
            if phone_elem:
                phone = self.clean_text(phone_elem.get_text())
        
        # 電郵: dt:contains("电子邮箱") + dd
        email = self._extract_dd_value(card, ['电子邮箱', '電子郵箱', 'Email'])
        # 如果是 Cloudflare 保護的
        ...
        address = self._extract_dd_value(card, ['相关地址', '相關地址', '地址'])
        # 清理地址中的「檢視地圖位置」等文字
        if address:
            address = re.sub(r'\s*\[.*地圖.*\]', '', address).strip()
    ...
    def _extract_dd_value(self, card, dt_contains) -> Optional[str]:
        """
        從 dt + dd 配對中提取值
        dt_contains 可以是字串或字串列表
        """
        if not card:
            return None
        
        # 支持單個標籤或多個標籤
        if isinstance(dt_contains, str):
            dt_contains = [dt_contains]
        
        dts = card.find_all('dt')
        for dt in dts:
            dt_text = dt.get_text()
            for label in dt_contains:
                if label in dt_text:
                    dd = dt.find_next_sibling('dd')
                    if dd:
